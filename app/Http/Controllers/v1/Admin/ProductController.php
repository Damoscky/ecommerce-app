...
class ProductController extends Controller
{
    ...
    public function importProducts(Request $request)
    {
        ...
        if ($validator->fails()) {
            return JsonResponser::send(true, 'Incorrect file format uploaded', []);
        }
        ...
        return JsonResponser::send(false, 'Product successfully uploaded', null);
    }
    ...
    public function listAllProducts(Request $request)
    {
        ...
        $products = Product::where("created_at", "!=", null)->orderBy('id', 'DESC')->with('category','user','subcategory','children')
        ->when($searchParam, function($query, $searchParam) use($request) {
            return $query->where('product_name', $searchParam);
        })
        ->get();

        toastr()->success($products->count() . ' Product(s) Available');
        return view('admin.all-products', compact('products'));
    }

    /**
     * fetch list of deactivated products
     */
    public function listDeactivatedProducts()
    {
        $products = Product::where("created_at", "!=", null)->where('is_active', 0)->orderBy('id', 'DESC')->with('category')->with('subcategory')->with('children')->with('user')->paginate(10);

        toastr()->success($products->count() . ' Product(s) Available');
        return view('admin.deactivated-products', compact('products'));
    }

    /**
     * fetch list of activated products
     */
    public function listActivatedProducts()
    {
        $products = Product::where("created_at", "!=", null)->where('is_active', 1)->orderBy('id', 'DESC')->with('category')->with('subcategory')->with('children')->with('user')->paginate(10);

        toastr()->success($products->count() . ' Product(s) Available');
        return view('admin.activated-products', compact('products'));
    }
    ...
    {
        $product = Product::where('id', $id)->with('category')->with('subcategory')->with('children')->with('user')->first();
    ...
    public function productStat()
    {
        ...
        $deactivatedProducts = Product::where('is_active', 0)->count();
        $activatedProducts = Product::where('is_active', 1)->count();
    ...
    public function productByStatus(Request $request, $status)
    {
        ...
        $products = Product::where('is_active', $status)->with('category')->with('subcategory')->with('children')->paginate(10);
    ...
    public function newProducts()
    {
        ...
        $data = Product::whereDate('created_at', '>', $twoWeeks)->paginate(12);
